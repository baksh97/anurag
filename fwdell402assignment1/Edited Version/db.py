...
class Db:
    def __init__(self):
        ...
        self.inst_password = self.passwords[-1]
        self.passwords.pop()
    ...
    def get_aggr_student(self, usr):
        return np.mean(self.get_marks_student(usr))
    ...
    def get_marks_all(self):
        mark_list = {}
        for name in self.names:
            mark_list[name] = self.marks[self.names[name]].astype(np.uint32)
        return mark_list
